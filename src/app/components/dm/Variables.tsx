...
const coreColors: Record<string, string> = {
  Req: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300',
  Exp: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300',
  Perm: 'bg-zinc-100 text-zinc-600 dark:bg-zinc-700/40 dark:text-zinc-400',
};
  ...
  return (
    <div className="h-full flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-zinc-900 dark:text-zinc-100">Variables</h1>
          <p className="text-[14px] text-zinc-500 dark:text-zinc-400 mt-1">Define and manage variables across entities and standards</p>
        </div>
      ...
      <div className="flex gap-3 items-center flex-wrap">
        ...
        <select value={filterStandard} onChange={e => setFilterStandard(e.target.value)} className="px-3 py-2.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg text-[14px] text-zinc-900 dark:text-zinc-100 outline-none">
          <option value="All">All Standards</option><option>CDISC</option><option>OMOP</option><option>Custom</option>
        </select>
        <select value={filterEntity} onChange={e => setFilterEntity(e.target.value)} className="px-3 py-2.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg text-[14px] text-zinc-900 dark:text-zinc-100 outline-none">
          <option value="All">All Entities</option>
          {entities.map(e => <option key={e} value={e}>{e}</option>)}
        </select>
        <select value={filterOrigin} onChange={e => setFilterOrigin(e.target.value)} className="px-3 py-2.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg text-[14px] text-zinc-900 dark:text-zinc-100 outline-none">
          <option value="All">All Origins</option><option>Standard</option><option>Standard-Modified</option><option>Custom</option>
        </select>
      </div>

      <p className="text-[13px] text-zinc-500 dark:text-zinc-400">{filtered.length} variables found</p>
      ...
      <div className="flex-1 overflow-auto bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl">
        <table className="w-full">
          ...
          <tbody>
            {filtered.map(v => (
              <tr key={v.id} className="border-b last:border-b-0 border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 cursor-pointer transition-colors" onClick={() => setSelectedVar(v)}>
                <td className="px-4 py-3 text-[14px] text-zinc-900 dark:text-zinc-100 font-mono">{v.name}</td>
                <td className="px-4 py-3 text-[13px] text-zinc-700 dark:text-zinc-300 max-w-[200px] truncate">{v.label}</td>
                <td className="px-4 py-3 text-[13px] text-zinc-600 dark:text-zinc-400 font-mono">{v.entity}</td>
                ...
                <td className="px-4 py-3 text-[13px] text-zinc-500 dark:text-zinc-400">{v.dataType}{v.length ? `(${v.length})` : ''}</td>
      ...
      {/* Detail */}
      {selectedVar && (
        <div className="fixed inset-0 bg-black/30 dark:bg-black/50 z-50 flex justify-end" onClick={() => setSelectedVar(null)}>
          <div className="w-[500px] bg-white dark:bg-zinc-900 h-full shadow-xl overflow-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
              <div>
                <h2 className="text-zinc-900 dark:text-zinc-100 font-mono">{selectedVar.name}</h2>
                <p className="text-[13px] text-zinc-500 dark:text-zinc-400">{selectedVar.label}</p>
              </div>
              <button onClick={() => setSelectedVar(null)} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 text-[20px]">&times;</button>
            </div>
            <div className="p-6 space-y-4">
              <div className="flex gap-2 flex-wrap">
                ...
                {selectedVar.required && <span className="text-[11px] px-2 py-0.5 rounded-full bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300">Required</span>}
              </div>
              {[
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/30 dark:bg-black/50 z-50 flex items-center justify-center" onClick={() => setShowCreate(false)}>
          <div className="w-[560px] bg-white dark:bg-zinc-900 rounded-2xl shadow-xl max-h-[80vh] overflow-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-zinc-200 dark:border-zinc-800">
              <h2 className="text-zinc-900 dark:text-zinc-100">Create New Variable</h2>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[12px] text-zinc-500 dark:text-zinc-400 mb-1 block">Name</label>
                  <input className="w-full px-3 py-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-[14px] text-zinc-900 dark:text-zinc-100 outline-none font-mono" placeholder="VARNAME" />
                </div>
                <div>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="text-[12px] text-zinc-500 dark:text-zinc-400 mb-1 block">Entity</label>
                  <select className="w-full px-3 py-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-[14px] text-zinc-900 dark:text-zinc-100 outline-none">
                    {entities.map(e => <option key={e}>{e}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-[12px] text-zinc-500 dark:text-zinc-400 mb-1 block">Data Type</label>
                  <select className="w-full px-3 py-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-[14px] text-zinc-900 dark:text-zinc-100 outline-none">
