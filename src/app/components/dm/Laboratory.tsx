...
const typeColors: Record<string, string> = {
  'Central Lab': 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300',
  'Reference Lab': 'bg-violet-100 text-violet-800 dark:bg-violet-900/40 dark:text-violet-300',
  'Specialty Lab': 'bg-cyan-100 text-cyan-800 dark:bg-cyan-900/40 dark:text-cyan-300',
  'Genomics Lab': 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/40 dark:text-indigo-300',
  'Academic Lab': 'bg-fuchsia-100 text-fuchsia-800 dark:bg-fuchsia-900/40 dark:text-fuchsia-300',
};
  ...
  return (
    <div className="h-full flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-zinc-900 dark:text-zinc-100">Laboratory</h1>
          <p className="text-[14px] text-zinc-500 dark:text-zinc-400 mt-1">Manage partner laboratories, accreditations, and contact information</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4">
          <p className="text-[12px] text-zinc-500 dark:text-zinc-400">Total Labs</p>
          <p className="text-[24px] text-zinc-900 dark:text-zinc-100 mt-1">{labs.length}</p>
        </div>
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4">
          <p className="text-[12px] text-zinc-500 dark:text-zinc-400">Active</p>
          <p className="text-[24px] text-emerald-600 dark:text-emerald-400 mt-1">{labs.filter(l => l.status === 'Active').length}</p>
        </div>
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4">
          <p className="text-[12px] text-zinc-500 dark:text-zinc-400">Countries</p>
          <p className="text-[24px] text-zinc-900 dark:text-zinc-100 mt-1">{new Set(labs.map(l => l.country)).size}</p>
        </div>
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4">
          <p className="text-[12px] text-zinc-500 dark:text-zinc-400">Avg Turnaround</p>
          <p className="text-[24px] text-zinc-900 dark:text-zinc-100 mt-1">{Math.round(labs.reduce((s, l) => s + l.turnaroundDays, 0) / labs.length)}d</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 content-start">
        {filtered.map(lab => (
          <div key={lab.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 cursor-pointer hover:shadow-md dark:hover:border-zinc-700 transition-all"
            onClick={() => setSelected(lab)}>
            <div className="flex items-start justify-between mb-3">
              <div className="min-w-0 flex-1">
                <p className="text-[15px] text-zinc-900 dark:text-zinc-100 truncate">{lab.name}</p>
                <p className="text-[12px] text-zinc-500 dark:text-zinc-400 font-mono">{lab.code}</p>
              </div>
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              ...
              {lab.specializations.length > 3 && (
                <span className="text-[10px] px-2 py-0.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 rounded-full">+{lab.specializations.length - 3}</span>
              )}
            </div>

            <div className="flex items-center justify-between text-[12px] text-zinc-500 dark:text-zinc-400 pt-3 border-t border-zinc-100 dark:border-zinc-800">
        ...
        <div className="fixed inset-0 bg-black/30 dark:bg-black/50 z-50 flex justify-end" onClick={() => setSelected(null)}>
          <div className="w-[520px] bg-white dark:bg-zinc-900 h-full shadow-xl overflow-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <p className="text-[12px] text-zinc-500 dark:text-zinc-400 mb-2">Specializations</p>
                <div className="flex flex-wrap gap-2">
              ...
              <div>
                <p className="text-[12px] text-zinc-500 dark:text-zinc-400 mb-1">Turnaround Time</p>
                <p className="text-[14px] text-zinc-900 dark:text-zinc-100">{selected.turnaroundDays} days</p>
              </div>
        ...
        <div className="fixed inset-0 bg-black/30 dark:bg-black/50 z-50 flex items-center justify-center" onClick={() => setShowCreate(false)}>
          <div className="w-[580px] bg-white dark:bg-zinc-900 rounded-2xl shadow-xl max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-zinc-200 dark:border-zinc-800">
              <h2 className="text-zinc-900 dark:text-zinc-100">Add Laboratory</h2>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-[12px] text-zinc-500 dark:text-zinc-400 mb-1 block">Type</label><input className="w-full px-3 py-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-[14px] text-zinc-900 dark:text-zinc-100 outline-none" placeholder="e.g. Central Lab, Genomics Lab" /></div>
                <div><label className="text-[12px] text-zinc-500 dark:text-zinc-400 mb-1 block">Turnaround (days)</label><input type="number" className="w-full px-3 py-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-[14px] text-zinc-900 dark:text-zinc-100 outline-none" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
