...
export function FileUploader() {
  ...
  return (
    <div className="flex flex-col gap-6 h-full w-full min-h-0 overflow-auto pb-8 max-w-5xl">
      <div>
        <h2 className="text-2xl font-bold text-zinc-900">File Uploader</h2>
        <p className="text-sm text-zinc-500 mt-1">Use the CLI tool or API to upload files programmatically to the data lake.</p>
      </div>
      ...
      <div className="glass-card rounded-2xl">
        <div className="px-5 py-4 border-b border-zinc-100 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-zinc-900">1. Create an API Key</p>
        ...
        {showNewKey && (
          <div className="px-5 py-4 border-b border-zinc-100 bg-zinc-50 flex items-center gap-3">
            <input type="text" value={newKeyName} onChange={e => setNewKeyName(e.target.value)} placeholder="Key name (e.g. CI/CD Pipeline)"
              className="flex-1 px-3 py-2 bg-white border border-zinc-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50" autoFocus
              onKeyDown={e => e.key === 'Enter' && createKey()} />
            <button onClick={createKey} disabled={!newKeyName.trim()} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-zinc-300 text-white rounded-lg text-sm font-medium transition-colors">Create</button>
            <button onClick={() => setShowNewKey(false)} className="px-3 py-2 text-sm text-zinc-500 hover:text-zinc-700">Cancel</button>
          </div>
        )}
        ...
        <div className="divide-y divide-zinc-100">
          {keys.map(key => (
            <div key={key.id} className={`px-5 py-4 flex items-center gap-4 ${key.status === 'revoked' ? 'opacity-50' : ''}`}>
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${key.status === 'active' ? 'bg-emerald-50' : 'bg-zinc-100'}`}>
                <Shield className={`w-4 h-4 ${key.status === 'active' ? 'text-emerald-600' : 'text-zinc-400'}`} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-medium text-zinc-900">{key.name}</p>
                  {key.status === 'revoked' && <span className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-500 font-medium">Revoked</span>}
                </div>
              ...
              <div className="flex items-center gap-1.5 flex-wrap shrink-0">
                {key.scopes.map(s => (
                  <span key={s} className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-500 font-medium">{s}</span>
                ))}
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => copyToClipboard(key.prefix, key.id)} className="p-1.5 text-zinc-400 hover:text-zinc-600 rounded-md hover:bg-zinc-100 transition-colors" title="Copy key">
                  {copiedId === key.id ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}
                </button>
                {key.status === 'active' && (
                  <button onClick={() => revokeKey(key.id)} className="p-1.5 text-zinc-400 hover:text-red-500 rounded-md hover:bg-zinc-100 transition-colors" title="Revoke key">
      ...
      <div className="glass-card rounded-2xl">
        <div className="px-5 py-4 border-b border-zinc-100 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-zinc-900">2. Install & Use the CLI</p>
              <p className="text-xs text-zinc-400">v2.4.1 — Command line tool for data upload and management</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <a href="#" className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-600 border border-blue-200 rounded-lg hover:bg-blue-50 transition-colors">
              <ExternalLink className="w-3 h-3" /> Documentation
            </a>
            <button className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-900 text-white rounded-lg text-xs font-medium hover:bg-zinc-800 transition-colors">
        ...
        <div className="p-5 space-y-3">
          {CLI_EXAMPLES.map((ex, i) => (
            <div key={i}>
              <p className="text-[11px] text-zinc-500 mb-1 uppercase tracking-wider">{ex.label}</p>
              <div className="flex items-center gap-2 bg-zinc-900 rounded-lg px-4 py-2.5 font-mono text-[13px] text-emerald-400 group">
      ...
      <div className="flex items-start gap-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl">
        ...
        <div className="text-xs text-amber-700">
          ...
          <p className="text-amber-600">API keys grant access to your data lake. Never share keys in public repositories or unsecured channels. Revoke unused keys regularly.</p>
        </div>
      </div>
    </div>
  );
