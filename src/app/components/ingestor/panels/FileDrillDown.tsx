...
import {
  ...
  AlertTriangle, CheckCircle2, Images, BookOpen, Search,
} from 'lucide-react';
...
type Tab = 'distribution' | 'values' | 'quality';

export function FileDrillDown({ node, onClose }: { node: EditorNode; onClose: () => void }) {
  return (
    <div className="flex flex-col h-full bg-white border-l border-zinc-200">
      <Header node={node} onClose={onClose} />
      <div className="flex-1 overflow-auto">
        {node.type === 'tabularFile' && <TabularBody node={node} />}
        {node.type === 'fileCollection' && <CollectionBody node={node} />}
...
function Header({ node, onClose }: { node: EditorNode; onClose: () => void }) {
  const d = node.data;
  const Icon = node.type === 'fileCollection' ? Images : node.type === 'contextNode' ? BookOpen : null;
  return (
    <div className="flex items-start gap-3 px-4 py-3 border-b border-zinc-200 shrink-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-mono text-sm font-semibold text-zinc-900 truncate flex items-center gap-1.5">
          {Icon && <Icon className="w-3.5 h-3.5 text-zinc-400" />}{d.label}
        </p>
...
// ─────────────────────────── TABULAR ───────────────────────────
function TabularBody({ node }: { node: EditorNode }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* lista variabili */}
      <div className="px-2 pb-2 max-h-56 overflow-auto">
        {filtered.map((v) => (
          <button
            key={v.name}
          ...
          >
            ...
            {(v.issues?.length ?? 0) > 0 && <AlertTriangle className="w-3 h-3 text-amber-500 shrink-0" />}
          </button>
        ))}
      </div>

      {/* dettaglio variabile selezionata */}
      {current && (
        <div className="border-t border-zinc-200 px-4 py-3">
          ...
          <div className="flex gap-1 mb-3 bg-zinc-100 p-0.5 rounded-lg">
            {([['distribution', 'Distribuzione'], ['values', 'Valori'], ['quality', 'Qualità']] as [Tab, string][]).map(
              ([id, label]) => (
                <button
                  key={id}
                  onClick={() => setTab(id)}
                  className={`flex-1 text-xs py-1 rounded-md transition-colors ${
                    tab === id ? 'bg-white shadow-sm text-zinc-900 font-medium' : 'text-zinc-500 hover:text-zinc-700'
                  }`}
                >
                  {label}
                </button>
          ...
          {tab === 'distribution' && <DistributionTab variable={current} />}
          {tab === 'values' && <ValuesTab variable={current} />}
          {tab === 'quality' && <QualityTab variable={current} />}
        </div>
      )}
    </div>
  );
...
function DistributionTab({ variable }: { variable: Variable }) {
  ...
  return (
    <div>
      ...
      {dist.kind === 'date' && (
        <p className="text-xs text-zinc-500 mt-2 text-center">{dist.min} → {dist.max}</p>
      )}
    </div>
  );
}
...
// ─────────────────────────── COLLECTION ───────────────────────────
function CollectionBody({ node }: { node: EditorNode }) {
  const d = node.data;
  const m = d.match;
  return (
    <div className="px-4 py-3 space-y-4">
      ...
      {m && (
        <div>
          <p className="text-[11px] font-medium text-zinc-500 mb-1.5">Match con i metadati (per nome)</p>
          ...
          <p className="text-[11px] font-medium text-zinc-500 mb-1">Esempi non associati</p>
          <ul className="space-y-1">
