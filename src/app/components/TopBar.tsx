...
import { Bell, User, Upload, AlertTriangle, CheckCircle, Info, X } from 'lucide-react';
...
const typeColor: Record<string, string> = {
  ingestion: 'bg-blue-100 text-blue-600',
  alert: 'bg-amber-100 text-amber-600',
  success: 'bg-emerald-100 text-emerald-600',
  info: 'bg-violet-100 text-violet-600',
};

export function TopBar({ currentTabLabel }: { currentTabLabel: string }) {
  ...
  return (
    <header className="h-16 bg-white border-b border-zinc-200 flex items-center justify-between px-6 transition-colors duration-200 shrink-0">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={panelRef}>
          <button
            ...
            className="p-2.5 text-zinc-500 hover:bg-zinc-100 rounded-full transition-colors relative"
            aria-label="Notifications"
          >
            <Bell className="w-4 h-4" />
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 flex items-center justify-center bg-red-500 rounded-full text-[10px] font-medium text-white border-2 border-white">
                {unreadCount}
              </span>
            )}
          </button>
          ...
          {isOpen && (
            <div className="absolute right-0 top-full mt-2 w-96 bg-white border border-zinc-200 rounded-2xl shadow-2xl z-50 overflow-hidden">
              {/* Header */}
              ...
              <div className="max-h-[400px] overflow-y-auto">
                {notifications.length === 0 ? (
                  ...
                  notifications.map(n => (
                    <div
                      key={n.id}
                      className={`flex items-start gap-3 px-4 py-3 border-b border-zinc-50 hover:bg-zinc-50 transition-colors cursor-pointer ${!n.read ? 'bg-blue-50/50' : ''}`}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <button
                            ...
                            className="p-0.5 text-zinc-400 hover:text-zinc-600 rounded shrink-0"
                          >
